...
void PlayWithComputer()
{
	...
	{	for (int a = 0; a <= l; a++) {		//fill BoardDimensions with whitespace
		for (int b = 0; b <= d; b++)	//
			BoardDimensions[a][b] = ' ';		//
	}								//
	...
	while (!gamewon) {//will stop when game is won, ! means NOT makes the opposite be checked
		...
		while (true) {//will run until 'break;'
			if (charsBoardDimensionsd == (l+1)*(d+1)) break;//if draw
			if (changePlay == 1)
				cin >> hold;//get user input
			else hold = I;
			hold--;//take off 1 to account for arrays starting at 0 not 1
			if (hold <= d && hold >= 0) break;//if within valid range stop loop
	...
	system("cls");//this clears the screen
	if (charsBoardDimensionsd == (l + 1) * (d + 1)) {//if draw
...
void PlayWithFriend()
{
	...
	while (!gamewon) {//will stop when game is won, ! means NOT makes the oppisite be checked
		...
		while (true) {//will run until 'break;'
			...
			hold--;//take off 1 to account for arrays starting at 0 not 1
			if (hold <= d && hold >= 0) break;//if within valid range stop loop
			else cout << "\nplease enter a value in the range of dimensions :";//ask for input and loop again
			if (cin.fail())	//catch a non number
			{						//
				cin.clear();		//Stops cin trying to put its value in to hold
				char c;			//Try entering a non number without this, 2 see what this does
		...
		if (charsBoardDimensionsd == (l + 1) * (d + 1)) break;//if draw
		hold2 = drop(hold, player);//drop the player store the row in hold2
		if (hold2 == -1)	cout << "Colom is full\nPlease enter anothor number in the range of dimensions:";//if error -1 row is full
		else {
			gamewon = check(hold2, hold);//check if game is run
...
void display() {
	for (int a = 0; a <= l; a++)
	{
		for (int b = 0; b <= d; b++) cout << char(218) << char(196) << char(191) << " ";
		cout << '\n';
		for (int b = 0; b <= d; b++) cout << char(179) << BoardDimensions[a][b] << char(179) << " ";
		cout << '\n';
		for (int b = 0; b <= d; b++) cout << char(192) << char(196) << char(217) << " ";
		cout << '\n';
	}
	cout << " 1   2   3   4   5   6   7\n \n";
}
bool check(int a, int b) {
	...
	int ii;//horizontal
	//check for vertical(|)
	for (i = a + 1; BoardDimensions[i][b] == player && i <= l; i++, vertical++);//Check down
	...
	for (ii = b + 1; BoardDimensions[a][ii] == player && ii <= d; ii++, horizontal++);//Check right
	if (horizontal >= 4) return true;
	//check for diagonal 1 (\)
	for (i = a - 1, ii = b - 1; BoardDimensions[i][ii] == player && i >= 0 && ii >= 0; diagonal1++, i--, ii--);//up and left
	for (i = a + 1, ii = b + 1; BoardDimensions[i][ii] == player && i <= l && ii <= d; diagonal1++, i++, ii++);//down and right
	if (diagonal1 >= 4) return true;
	//check for diagonal 2(/)
	for (i = a - 1, ii = b + 1; BoardDimensions[i][ii] == player && i >= 0 && ii <= d; diagonal2++, i--, ii++);//up and right
	for (i = a + 1, ii = b - 1; BoardDimensions[i][ii] == player && i <= l && ii >= 0; diagonal2++, i++, ii--);//up and left
